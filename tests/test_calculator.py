...
def test_readline_import_fallback_to_builtin(monkeypatch):
    """Ensure module falls back to builtin `readline` when `pyreadline3` is absent."""
    import importlib, types, sys, builtins

    # Ensure real pyreadline3 is not importable; provide a fake builtin readline
    monkeypatch.setitem(sys.modules, 'readline', types.ModuleType('readline'))
    ...
    assert calc.readline is sys.modules['readline']


def test_readline_import_none_when_unavailable(monkeypatch):
    """Ensure `readline` is set to None when neither module is available."""
    import importlib, sys, builtins

    real_import = builtins.__import__

    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name in ('pyreadline3', 'readline'):
            raise ImportError
        return real_import(name, globals, locals, fromlist, level)

    monkeypatch.setattr(builtins, '__import__', fake_import)
    monkeypatch.delitem(sys.modules, 'pyreadline3', raising=False)
    monkeypatch.delitem(sys.modules, 'readline', raising=False)

    import app.calculator as calc
    importlib.reload(calc)

    assert calc.readline is None
